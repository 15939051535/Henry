...
class BasePack(object):
    ...
    def send_keys1(self, locator, text, is_clear=True):
        '''
        发送文本，不清空连续输入
        Usage:
        locator = ("id","xxx")
    ...
    def clear_keys(self, locator, is_clear=True):
        '''只清空输入框的text文本'''
        element = self.find_element(locator)
        if is_clear == True:
            element.clear()
    ...
    def get_text1(self, locator, value):
        '''获取属性：方法二：获取input中value的值
...
if __name__ == '__main__':
    ...
    # driver = webdriver.Firefox()    # 实例1 driver
    # base_driver = BasePage(driver)  # 实例2 base_driver
    # base_driver.open("https://www.baidu.com")
    # input_loc = ("id", "kw")
    # base_driver.send_keys(input_loc, "yoyo")
    # button_loc = ("id", "su")
    # base_driver.click(button_loc)
    # print base_driver.get_title()
    # 调用driver里面的方法
    # driver.quit()
